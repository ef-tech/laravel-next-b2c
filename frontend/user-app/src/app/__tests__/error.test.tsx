...
// Mock next-intl
jest.mock("next-intl", () => {
  // Create a translation function that can be passed to NetworkError.getDisplayMessage()
  const createTranslationFunction = (namespace: string) => {
    const translationFn = (key: string) => {
      // Return mock translation based on current locale context
      const translations = (global as any).__MOCK_TRANSLATIONS__ || {};
      const fullKey = `${namespace}.${key}`;
      return translations[fullKey] || fullKey;
    };
    // Make it compatible with NetworkError.getDisplayMessage(t) which expects (key: string) => string
  ...
  return {
    ...
    NextIntlClientProvider: ({
      ...
      // Store messages in global context for useTranslations mock
      ...
      (global as any).__MOCK_TRANSLATIONS__ = flattenMessages(messages);
      return <>{children}</>;
    },
  };
});
...
describe("Error Boundary (User App)", () => {
  ...
  describe("Japanese locale (ja)", () => {
    // Import NextIntlClientProvider from mocked module
