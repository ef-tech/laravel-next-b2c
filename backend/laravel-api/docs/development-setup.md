...
| サービス | ポート | 用途 | アクセス方法 |
|---------|--------|------|-------------|
| Laravel API | 13000 | メインアプリケーション | http://localhost:13000 |
| PostgreSQL | 5432（内部） | データベース | Docker exec経由のみ（ホストアクセス不可） |
| Redis | 6379（内部） | キャッシュ・セッション | Docker exec経由のみ（ホストアクセス不可） |
| Mailpit | 1025（SMTP）、8025（UI） | 開発用メール | SMTPは内部のみ、UIは外部ポート経由 |
| MinIO | 9000（API）、9001（Console） | オブジェクトストレージ | 外部ポート経由でアクセス可能 |
