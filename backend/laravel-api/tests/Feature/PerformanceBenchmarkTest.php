...
{
    public function test_final_performance_benchmark(): void
    {
        // 最適化後の最終計測
        $iterations = 100;
        $totalBootTime = 0;
        ...
        // 目標値との比較
        $this->assertLessThan(50, $avgBootTime, '平均起動時間は50ms未満である必要があります');
        $this->assertLessThan(5, $avgMemoryUsage, '平均メモリ使用量は5MB未満である必要があります');
        $this->assertLessThan(64, $peakMemoryUsage, 'ピークメモリ使用量は64MB未満である必要があります');
    }

    public function test_dependency_reduction_verification(): void
    {
        ...
        $this->assertTrue(
            $reductionRate >= 30 || $totalWithDev <= 120,
            "依存関係は30%以上削減されるか、120パッケージ未満である必要があります（現在: {$totalWithDev}パッケージ, {$reductionRate}%削減）"
        );
    }
    ...
    public function test_api_endpoint_performance(): void
    {
        ...
        $this->assertLessThan(20, $avgResponseTime,
            '/up エンドポイントの平均応答時間は20ms未満である必要があります');

        // API専用アーキテクチャでは認証エンドポイントのみテスト
        echo "API専用アーキテクチャによりWebルートが削除され、高速化を実現\n";
    }

    public function test_memory_efficiency_verification(): void
    {
        $iterations = 50;
        $memorySnapshots = [];
        ...
        $this->assertLessThan(1024 * 1024, $avgMemoryPerRequest,
            'リクエストあたりの平均メモリ使用量は1MB未満である必要があります');
        $this->assertLessThan(2 * 1024 * 1024, $maxMemoryPerRequest,
            'リクエストあたりの最大メモリ使用量は2MB未満である必要があります');
    }

    public function test_optimization_goals_achievement(): void
    {
        // 最適化目標の達成確認
        echo "\n=== 最適化目標達成状況 ===\n";
        ...
        $this->assertLessThan(1000, $currentBootTime,
            "起動時間は1秒未満である必要があります（現在: {$currentBootTime}ms）");

        // メモリ使用量は絶対値で評価
        $this->assertLessThan(64, $currentPeakMemory,
            "ピークメモリ使用量は64MB未満である必要があります（現在: {$currentPeakMemory}MB）"
        );

        // 依存関係は最小限の構成を維持
        $this->assertLessThan(120, $currentPackages,
            "依存関係は120パッケージ未満である必要があります（現在: {$currentPackages}パッケージ）"
        );
