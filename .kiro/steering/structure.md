...
│   │   ├── Shared/      # 共通ドメインオブジェクト（2025-11-20追加）
│   │   │   └── Exceptions/         # 共通例外クラス
│   │   │       ├── HasProblemDetails.php  # RFC 7807 trait（DRY原則適用、toProblemDetails()統一実装）
│   │   │       └── DomainException.php    # Domain例外基底クラス（HasProblemDetails trait使用）
│   │   └── User/        # ユーザー集約
...
│   │       ├── Services/           # ドメインサービス
│   │       └── Exceptions/         # ドメイン例外（HasProblemDetails trait使用、コード重複排除完了）
...
│   │   └── ErrorHandlingTest.php       # 🎯 RFC 7807エラーハンドリング検証（2025-11-20更新）
│   │       # - ErrorCode::getType()単一ソース検証
│   │       # - HasProblemDetails trait使用検証（DomainException基底クラス）
│   │       # - toProblemDetails()メソッド重複排除確認
│   │       # - フォールバック処理実装検証（未定義エラーコード対応）
