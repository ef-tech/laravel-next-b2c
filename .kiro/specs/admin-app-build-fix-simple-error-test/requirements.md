...
2. **TypeScript型エラー**: `src/i18n.ts` で `locale` が `string | undefined` 型だが、`RequestConfig` は `locale: string`（必須）を要求している
...
### Requirement 2: next-intl i18n.ts型エラーの修正

**Objective:** 開発者として、`src/i18n.ts` のTypeScript型エラーが解消され、`getRequestConfig` の戻り値が `RequestConfig` 型と完全に互換性があることを期待する

#### Acceptance Criteria

1. **WHEN** `getRequestConfig` が `locale` パラメータを受け取る **THEN** Admin App **SHALL** `locale` が `undefined` の場合でも `defaultLocale` を使用して `string` 型を保証する
2. **WHEN** `getRequestConfig` の戻り値の `locale` プロパティを検証する **THEN** Admin App **SHALL** `locale: string`（必須、非undefined）型であることを保証する
3. **WHEN** TypeScriptコンパイル（`npx tsc --noEmit`）を実行する **THEN** Admin App **SHALL** `src/i18n.ts` に関する型エラーを出さない
4. **IF** `locale` パラメータが `undefined` または無効な値の場合 **THEN** Admin App **SHALL** 自動的に `i18nConfig.defaultLocale`（'ja'）を使用する
5. **WHEN** `next-intl` の `RequestConfig` 型定義と照合する **THEN** Admin App **SHALL** 戻り値の型が完全に一致する

### Requirement 3: 本番ビルドの検証とテスト
...
### Requirement 4: CI/CDパイプラインとの整合性
...
2. **WHEN** `i18n.ts` の型修正が完了する **THEN** Admin App **SHALL** コード内にTypeScript型注釈やコメントで明確な型情報を記載する
3. **WHEN** 修正理由を記録する **THEN** 開発チーム **SHALL** コミットメッセージにIssue #124への参照を含める
4. **WHERE** プロジェクトのステアリングドキュメント（`.kiro/steering/`）を更新する際 **THE** 開発チーム **SHALL** Next.js App Routerのレイアウト要件とi18n統合のベストプラクティスを記載する

### Requirement 6: 後方互換性とリグレッションの防止
...
2. **WHEN** `i18n.ts` を修正する **THEN** Admin App **SHALL** 既存のロケール検出ロジック（Cookie、Accept-Language header）を維持する
3. **WHEN** Admin Appの全Jestテストを実行する **THEN** Admin App **SHALL** 全テストがパスする（修正前のテストカバレッジを維持）
4. **WHEN** User Appの本番ビルドを実行する **THEN** User App **SHALL** ビルドが成功する（Admin Appの修正が影響しない）
5. **WHERE** 同様のファイル構造がUser Appに存在する場合 **THE** User App **SHALL** 同じレイアウト要件違反やTypeScript型エラーを持たない

## 技術スタック

**Frontend**: Next.js 15.5, React 19, TypeScript, App Router, next-intl
...
# Pull Request作成後、CI/CDワークフローが自動実行される
```

## 参考情報

- **Next.js App Router Layout Requirements**: https://nextjs.org/docs/app/building-your-application/routing/pages-and-layouts
- **next-intl getRequestConfig**: https://next-intl-docs.vercel.app/docs/usage/configuration#i18nts
- **検出コンテキスト**: Issue #117 (frontend-lib-monorepo-consolidation) Task 7実行時
- **関連Issue**: #127 - CI/CD: フロントエンド本番ビルドステップ欠落
