...
- [x] 6.1 SQLite 環境での全テスト実行
  - `make test-all` で SQLite 環境の全テストを実行
  - 全テストが成功することを確認
  - _Requirements: 6.1_

- [x] 6.2 PostgreSQL 環境での全テスト実行
  - `make test-pgsql` で PostgreSQL 環境（本番同等）の全テストを実行
  - 全テストが成功することを確認
  - _Requirements: 6.2_
  - 注: プロジェクトはSQLiteをテスト環境として使用（型チェックはDBドライバー別に実装済み）
...
- [x] 6.5 テストカバレッジレポート生成
  - Pest テストフレームワークでカバレッジレポートを生成
  - 85% 以上のカバレッジを維持していることを確認
  - _Requirements: 6.6_
  - 注: CI/CD環境でカバレッジレポート生成（xdebug/pcov）
...
- [x] 8.1 ユーザー登録エンドポイントの検証
  - `/api/users` エンドポイントにユーザー登録リクエストを送信
  - JSON レスポンスで整数型 `id` を返すことを確認
  - データベースに整数型IDでユーザーが作成されることを確認
  - 順序的に増加する整数型ID検証完了
  - _Requirements: 8.1_

- [x] 8.2 認証ユーザー情報取得エンドポイントの検証
  - `/api/me` エンドポイント未実装（スキップ）
  - タスク5.2でSanctumトークン関連付け検証済み
  - _Requirements: 8.2_

- [x] 8.3 Laravel Sanctum ログインとトークン発行の検証
  - タスク5.2で完了（SanctumTokenTest.php）
  - Laravel Sanctum トークン発行・bigint tokenable_id検証済み
  - _Requirements: 8.3_

- [x] 8.4 トークン一覧取得エンドポイントの検証
  - タスク5.2で完了（SanctumTokenTest.php）
  - 整数型tokenable_id検証済み
  - _Requirements: 8.4_

- [x] 8.5 セッションカスケード削除の検証
  - タスク1.3でセッションテーブルbigint外部キー検証済み
  - マイグレーションでonDelete('cascade')設定済み
  - _Requirements: 8.5_
