...
- [x] 9. バックエンドFeature Testsを実装する
- [x] 9.1 Exception Handler統合テストを実装する
...
- [x] 11. E2E Testsを実装する
- [x] 11.1 APIエラー表示E2Eテストを実装する
  - RFC 7807情報（title, detail, errorCode, requestId）の画面表示テストを実装する
  - Error Boundary UIの表示検証テストを実装する
  - Request IDのサポート用参照ID表示テストを実装する
  - _Requirements: 9.4_
  - _Note: e2e/projects/shared/tests/error-handling.spec.ts に3テスト実装完了。_

- [x] 11.2 バリデーションエラー表示E2Eテストを実装する
  - フィールド別エラーメッセージ画面表示テストを実装する
  - errorsフィールド解析・表示テストを実装する
  - _Requirements: 9.4_
  - _Note: 2テスト実装完了。バリデーションエラー詳細表示を検証。_

- [x] 11.3 認証エラー（401）リダイレクトE2Eテストを実装する
  - 401エラー発生時のログインページリダイレクトテストを実装する
  - 認証エラーメッセージ表示テストを実装する
  - _Requirements: 9.4_
  - _Note: 2テスト実装完了。ログインページ未実装のためスキップ設定済み。_

- [x] 11.4 ネットワークエラー表示E2Eテストを実装する
  - ネットワークエラーメッセージ表示テストを実装する
  - 再試行ボタン表示・動作テストを実装する
  - NetworkError検出・UI表示テストを実装する
  - _Requirements: 9.4_
  - _Note: 3テスト実装完了。タイムアウト・接続エラーを検証。_

- [x] 11.5 500エラーマスキングE2Eテストを実装する
  - 本番環境設定時の内部エラー詳細マスキングテストを実装する
  - 汎用エラーメッセージ表示テストを実装する
  - Request ID表示維持テストを実装する
  - _Requirements: 9.4_
  - _Note: 3テスト実装完了。Error ID表示を検証。_

- [x] 11.6 再試行ボタン動作E2Eテストを実装する
  - Error Boundaryの再試行ボタンクリックテストを実装する
  - reset()関数呼び出しテストを実装する
  - router.refresh()またはreset()によるリカバリーテストを実装する
  - _Requirements: 9.4_
  - _Note: 3テスト実装完了。reset()によるリカバリーを検証。_

- [x] 11.7 Request ID表示E2Eテストを実装する
  - Error Boundary UIのRequest ID（trace_id）表示テストを実装する
  - サポート問い合わせ用参照ID提示テストを実装する
  - _Requirements: 9.4_
  - _Note: 2テスト実装完了。UUID形式検証・サポートメッセージ表示確認。_

- [x] 12. パフォーマンステストを実装する
- [x] 12.1 Exception生成コストテストを実装する
  - DomainException生成時のオーバーヘッド計測テストを実装する
  - 5ms以下の目標達成を検証する
  - _Requirements: 非機能要件パフォーマンス.1_
  - _Note: 7テスト実装完了。DomainException/ApplicationException/InfrastructureException生成コスト測定、toProblemDetails()実行時間測定、連続実行安定性検証。全目標達成。_

- [x] 12.2 翻訳処理オーバーヘッドテストを実装する
  - Laravel Translation Cache効果計測テストを実装する
  - 2回目以降のアクセスが1ms以下であることを検証する
  - _Requirements: 非機能要件パフォーマンス.2_
  - _Note: 8テスト実装完了。1回目10ms以内・2回目以降1ms以下のキャッシュ効果検証、日英切り替えパフォーマンス測定、1000回負荷テスト。全目標達成。_

- [x] 12.3 RFC 7807変換コストテストを実装する
  - toProblemDetails()メソッド実行時のオーバーヘッド計測テストを実装する
  - 3ms以下の目標達成を検証する
  - _Requirements: 非機能要件パフォーマンス_
  - _Note: 9テスト実装完了。toProblemDetails()実行コスト3ms以下検証、各フィールド生成コスト測定、1000回混合変換負荷テスト。全目標達成。_

## Phase 5: ドキュメント・CI/CD統合

- [x] 13. ドキュメントを作成する
- [x] 13.1 エラーコード一覧ドキュメントを作成する
  - カテゴリー別エラーコード一覧表を作成する
  - 認証（AUTH-*）、バリデーション（VAL-*）、ビジネスロジック（BIZ-*）、インフラ（INFRA-*）を分類する
  - 各エラーコードのレスポンス例を記載する
  - HTTPステータスコード・メッセージ・対処方法を記載する
  - _Requirements: 10.1_
  - _Note: error-codes.md作成完了。RFC 7807形式説明・エラーコード命名規則・5カテゴリ13エラーコード定義（AUTH/VAL/BIZ/INFRA/NETWORK）・各エラーのHTTPステータス・日英メッセージ・発生条件・レスポンス例・トラブルシューティング手順・Request ID追跡方法・多言語対応詳細記載。_

- [x] 13.2 トラブルシューティングガイドを作成する
  - よくある問題と解決策を記載する
  - Request ID追跡方法を説明する
