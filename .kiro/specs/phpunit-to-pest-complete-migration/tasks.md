...
- [ ] 5. Featureテストの手動精緻化（Batch 2: 次の5ファイル）
- [ ] 5.1 アサーション構文のPest形式への変換
  - PHPUnitアサーション（`$this->assert*`）をPestの`expect()`構文に変換する
  - 複雑なアサーションチェーンをPestの流暢なAPIに変換する
  - Laravelテストヘルパー（`$this->get()`, `$this->postJson()`等）はそのまま維持する
  - setUp/tearDownメソッドをbeforeEach/afterEach関数に変換する（存在する場合）
...
- [ ] 5.2 ファイル単位でのテスト実行と検証
  - 変換した5ファイルそれぞれに対してPestテストを個別実行する
  - すべてのテストケースがグリーン状態であることを確認する
  - エラーが発生した場合、バックアップから復元し再変換する
  - _要件: 2.1, 2.2_

- [ ] 6. Featureテストの手動精緻化（Batch 3: 次の5ファイル）
- [ ] 6.1 アサーション構文のPest形式への変換
  - PHPUnitアサーション（`$this->assert*`）をPestの`expect()`構文に変換する
  - 複雑なアサーションチェーンをPestの流暢なAPIに変換する
  - Laravelテストヘルパー（`$this->get()`, `$this->postJson()`等）はそのまま維持する
  - setUp/tearDownメソッドをbeforeEach/afterEach関数に変換する（存在する場合）
...
- [ ] 6.2 ファイル単位でのテスト実行と検証
  - 変換した5ファイルそれぞれに対してPestテストを個別実行する
  - すべてのテストケースがグリーン状態であることを確認する
  - エラーが発生した場合、バックアップから復元し再変換する
  - _要件: 2.1, 2.2_

- [ ] 7. Featureテストの手動精緻化（Batch 4: 残り4ファイル）
- [ ] 7.1 アサーション構文のPest形式への変換
  - PHPUnitアサーション（`$this->assert*`）をPestの`expect()`構文に変換する
  - 複雑なアサーションチェーンをPestの流暢なAPIに変換する
  - Laravelテストヘルパー（`$this->get()`, `$this->postJson()`等）はそのまま維持する
  - setUp/tearDownメソッドをbeforeEach/afterEach関数に変換する（存在する場合）
...
- [ ] 8. Unitテストの手動精緻化
- [ ] 8.1 アサーション構文のPest形式への変換
  - PHPUnitアサーション（`$this->assert*`）をPestの`expect()`構文に変換する
  - シンプルなUnitテストのため、基本的なアサーション変換のみ実施する
  - _要件: 1.3, 2.2_

- [ ] 8.2 ファイル単位でのテスト実行と検証
  - 変換したUnitテストファイルに対してPestテストを個別実行する
  - すべてのテストケースがグリーン状態であることを確認する
  - _要件: 2.1, 2.2_

- [ ] 9. データベーステストとSanctum認証テストの検証
...
- [ ] 13. PHPUnit設定ファイルとComposerコマンドの削除
